...
// Prebuilt at module scope so SliderField never constructs Intl on render.
const numberFormattersByDigits = {
  default: new Intl.NumberFormat("en"),
  0: new Intl.NumberFormat("en", { maximumFractionDigits: 0 }),
  1: new Intl.NumberFormat("en", { maximumFractionDigits: 1 }),
...
function getNumberFormatter(options?: Intl.NumberFormatOptions) {
  ...
  if (digits === 0 || digits === 1 || digits === 2) {
    return numberFormattersByDigits[digits];
  }
  return numberFormattersByDigits.default;
}
...
function SliderField({ name, field, isPending }: AdditionalFieldProps) {
  ...
  const formatter = getNumberFormatter(field.formatOptions);
    ...
    <Field data-invalid={!!error}>
      ...
      <div className="relative flex gap-2">
        ...
        <Popover open={open} onOpenChange={setOpen}>
          ...
          >
            {date ? format(date, "PPP", { locale: dateLocale }) : <span>{field.placeholder}</span>}
