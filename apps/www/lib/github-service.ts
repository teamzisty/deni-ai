...
export class GitHubService {
  ...
  async createBranch(owner: string, repo: string, branchName: string, baseBranch: string = 'main'): Promise<void> {
    ...
    const baseBranchData = await this.makeRequest(`/repos/${owner}/${repo}/git/refs/heads/${baseBranch}`);
  ...
  async commitFiles(
    ...
    const branchData = await this.makeRequest(`/repos/${owner}/${repo}/git/refs/heads/${branchName}`);
    const latestCommitSha = branchData.object.sha;

    // Get the tree SHA from the latest commit
    const commitData = await this.makeRequest(`/repos/${owner}/${repo}/git/commits/${latestCommitSha}`);
    ...
    const newTreeData = await this.makeRequest(`/repos/${owner}/${repo}/git/trees`, {
      method: 'POST',
      body: JSON.stringify({
    ...
    // Create new commit
    const newCommitData = await this.makeRequest(`/repos/${owner}/${repo}/git/commits`, {
      method: 'POST',
      body: JSON.stringify({
  ...
  static async extractWebContainerFiles(
    chatId: string,
    excludePatterns: string[] = ['node_modules', '.git', 'dist', 'build', '.next', '.cache']
  ): Promise<FileChange[]> {
    try {
      // Import WebContainer utilities dynamically to avoid circular dependencies
      const { getWebContainerInstance, buildFileStructure } = await import('@/components/WebContainer');
      ...
      const extractFilesFromStructure = async (structure: any, currentPath: string = '') => {
      ...
      return files;    } catch (error: any) {
      console.error('Error extracting WebContainer files:', error);
      throw new Error(`Failed to extract WebContainer files: ${error?.message || 'Unknown error'}`);
    }
  }
  ...
  static generatePullRequestBody(files: FileChange[]): string {
    const fileList = files.map(file => `- ${file.operation}: \`${file.path}\``).join('\n');
