...
export function HubsProvider({ children }: { children: ReactNode }) {
  ...
  const setHubs = useCallback(
    (newHubs: Hub[] | ((prevState: Hub[]) => Hub[])) => {
      setInternalHubs((prevInternalHubs) => {
        const updatedHubs =
          typeof newHubs === "function" ? newHubs(prevInternalHubs) : newHubs;
  ...
  const normalizeHubTimestampProperties = (
    ...
    const defaults: Omit<
      Hub,
      "id" | "createdAt" | "updatedAt" | "fileReferences" | "chatSessionIds"
    > = {
      name: "",
      description: "",
      customInstructions: "",
    ...
    const normalized = {
      ...
      createdAt:
        typeof hubData.createdAt === "string" ||
        typeof hubData.createdAt === "number"
          ? new Date(hubData.createdAt).getTime()
          : ensureCurrentTime
            ? now
            : now,
      updatedAt:
        typeof hubData.updatedAt === "string" ||
        typeof hubData.updatedAt === "number"
          ? new Date(hubData.updatedAt).getTime()
          : ensureCurrentTime
            ? now
            : now,
      fileReferences: Array.isArray(hubData.fileReferences)
        ? hubData.fileReferences.map((fr: any) => ({
            id: fr.id || uuidv4(),
            name: fr.name || "",
            type: fr.type || "",
            path: fr.path || "",
            size: fr.size || 0,
            createdAt:
              typeof fr.createdAt === "string" ||
              typeof fr.createdAt === "number"
                ? new Date(fr.createdAt).getTime()
                : ensureCurrentTime
                  ? now
                  : now,
          }))
        : [],
      chatSessionIds: Array.isArray(hubData.chatSessionIds)
        ? hubData.chatSessionIds
        : [],
    };
    ...
    if (!Array.isArray(normalized.chatSessionIds))
      normalized.chatSessionIds = [];
    if (!Array.isArray(normalized.fileReferences))
  ...
  const loadHubsFromSupabase = useCallback(async () => {
    ...
    try {
      ...
      const loadedHubs: Hub[] = (data || []).map((item: any) =>
  ...
  }, [user, t, setHubs, setLoadingState]); // 初期化effect - 一度だけ実行される
  useEffect(() => {
  ...
  }, [user, isInitialized, loadHubsFromSupabase, setHubs]);
  const saveHubsToStorage = useCallback(
    async (hubsToSave: Hub[], specificIds?: Set<string>) => {
  ...
  const syncLocalHubsToSupabase = useCallback(
    async (localHubsToSync?: Hub[]) => {
      if (!user) {
        setLoadingState(false);
        return;
      }
      const hubsToProcess =
        localHubsToSync ||
        (
          JSON.parse(localStorage.getItem(LOCAL_STORAGE_KEY) || "[]") as Hub[]
        ).map((h) => normalizeHubTimestampProperties(h, true));
      ...
      try {
                ...
                normalizeHubTimestampProperties(
                  {
                    ...
                    customInstructions: item.custom_instructions,
                    chatSessionIds: item.chat_session_ids || [],
                    fileReferences: (item.file_references || []).map(
                      (fr: any) => ({
                        id: fr.id,
                        name: fr.name,
                        type: fr.type,
        ...
        for (const localHub of hubsToProcess) {
          ...
          if (supabaseHub) {
            // Hub exists in Supabase, only update if local is newer
  ...
  const syncHubs = useCallback(async () => {
    ...
    try {
      ...
      const supabaseHubs: Hub[] = (supabaseData || []).map((item: any) =>
        normalizeHubTimestampProperties(
          {
            id: item.id,
            name: item.name,
      ...
      const allHubIds = new Set([
        ...localHubs.map((h: Hub) => h.id),
        ...supabaseHubs.map((h: Hub) => h.id),
      ]);
      ...
      for (const id of allHubIds) {
        ...
        if (localHub && supabaseHub) {
          ...
          const hubToKeep =
            localHub.updatedAt >= supabaseHub.updatedAt
              ? localHub
              : supabaseHub;
          ...
          if (
            hubToKeep.updatedAt > (supabaseHub.updatedAt || 0) ||
  ...
  // Sync on login - ログイン状態の変化のみを監視
  useEffect(() => {
    if (prevAuthState === null) {
      // 初回実行時はスキップ（初期化effectで処理される）
  ...
  const getHub = (id: string): Hub | undefined => {
    ...
    return foundHub
      ? normalizeHubTimestampProperties(foundHub, true)
      : undefined;
  };
  ...
  const createHub = async (
    ...
    const updatedHubs = [
      ...hubs,
      normalizeHubTimestampProperties(newHub, true),
    ];
    setHubs(updatedHubs);
