...
import { ArrowLeft, MessageSquare, FileText, Save, Trash2, Paperclip, Link2 } from 'lucide-react';
...
const HubDetail: React.FC<HubDetailProps> = ({ hubId, onClose }) => {
  const { getHub, updateHub, deleteHub, addChatSessionToHub, removeChatSessionFromHub, addFileToHub, removeFileFromHub } = useHubs();
  ...
  const handleRemoveChatSession = useCallback((sessionId: string) => {
    if (hub) {
  ...
  const handleAddFile = useCallback(() => {
    if (hub && fileToLinkName.trim()) {
      ...
      addFileToHub(hub.id, { name: fileToLinkName, type: fileToLinkType, path: 'simulated/path/' + fileToLinkName, size: Math.floor(Math.random() * 100000) });
      setHub(getHub(hub.id) || null); // Refresh hub data
      setFileToLinkName('');
  ...
  return (
    <div className="p-4 md:p-6 h-full flex flex-col">
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-bold truncate" title={hub.name}>{hub.name}</h1>
      </div>

      <div className="flex-grow overflow-y-auto space-y-6 pb-16"> {/* Added pb-16 for footer spacing */}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              <label htmlFor="hubName" className="block text-sm font-medium mb-1">{t('hubNameLabel')}</label>
              <Input id="hubName" value={name} onChange={(e) => setName(e.target.value)} placeholder={t('hubNamePlaceholder')} />
            </div>
            <div>
              <label htmlFor="hubDescription" className="block text-sm font-medium mb-1">{t('hubDescriptionLabel')}</label>
              <Textarea id="hubDescription" value={description} onChange={(e) => setDescription(e.target.value)} placeholder={t('hubDescriptionPlaceholder')} rows={3} />
            </div>
            <div>
              <label htmlFor="hubInstructions" className="block text-sm font-medium mb-1">{t('hubCustomInstructionsLabel')}</label>
              <Textarea id="hubInstructions" value={customInstructions} onChange={(e) => setCustomInstructions(e.target.value)} placeholder={t('hubCustomInstructionsPlaceholder')} rows={5} />
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {hub.chatSessionIds.length === 0 ? (
              <p className="text-sm text-muted-foreground">{t('noLinkedChats')}</p>
            ) : (
              <ul className="space-y-2">
                {hub.chatSessionIds.map(sessionId => (
                  <li key={sessionId} className="flex justify-between items-center p-2 border rounded">
                    <span className="truncate" title={getChatSessionTitle(sessionId)}>
                      <MessageSquare size={16} className="inline mr-2" />
                      {getChatSessionTitle(sessionId)}
                    </span>
                    <Button variant="ghost" size="sm" onClick={() => handleRemoveChatSession(sessionId)} title={t('unlinkChat')}>
                      <Trash2 size={16} />
                    </Button>
                  </li>
                ))}
            ...
            <div className="mt-4 flex gap-2">
              ...
              <datalist id="chat-session-suggestions">
                {allChatSessions
                  ...
                  .map(cs => <option key={cs.id} value={cs.id}>{cs.title}</option>)}
              </datalist>
              <Button onClick={handleAddChatSession}><Link2 size={16} className="mr-1" /> {t('linkChat')}</Button>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {hub.fileReferences.length === 0 ? (
              <p className="text-sm text-muted-foreground">{t('noLinkedFiles')}</p>
            ) : (
              <ul className="space-y-2">
                {hub.fileReferences.map(file => (
                  <li key={file.id} className="flex justify-between items-center p-2 border rounded">
                    <span className="truncate" title={file.name}>
                      ...
                      {file.name} ({file.type}, {file.size ? (file.size / 1024).toFixed(1) + ' KB' : 'N/A'})
                    </span>
                    <Button variant="ghost" size="sm" onClick={() => handleRemoveFile(file.id)} title={t('removeFile')}>
                      <Trash2 size={16} />
                    </Button>
                  </li>
                ))}
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="flex gap-2">
                <Input value={fileToLinkName} onChange={(e) => setFileToLinkName(e.target.value)} placeholder={t('fileNamePlaceholder')} />
                <Input value={fileToLinkType} onChange={(e) => setFileToLinkType(e.target.value)} placeholder={t('fileTypePlaceholder')} />
                <Button onClick={handleAddFile}><Paperclip size={16} className="mr-1" /> {t('addFile')}</Button>
              </div>
              <p className="text-xs text-muted-foreground">{t('fileReferenceNote')}</p>
            </div>
          </CardContent>
        </Card>
      </div>
