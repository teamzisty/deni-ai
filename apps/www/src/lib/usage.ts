...
export const MODEL_LIMITS: Record<string, UsageLimit> = Object.entries(models).reduce((acc, [modelKey, modelData]) => {
...
export async function getAllUsage(userId: string, onlyUsedModels: boolean = false): Promise<UsageInfo[]> {
  ...
  const modelsToProcess = onlyUsedModels 
    ? Object.entries(MODEL_LIMITS).filter(([modelKey, config]) => usageMap.has(modelKey) || config.premium)
