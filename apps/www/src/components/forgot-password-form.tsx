...
export function ForgotPasswordForm({ className, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : t('auth.forgotPassword.errorOccurred'))
    } finally {
      setIsLoading(false)
    }
  }
  ...
  return (
    <div className={cn('flex flex-col gap-6', className)} {...props}>
      {success ? (
        <Card>
          <CardHeader>
            <CardTitle className="text-2xl">{t('auth.forgotPassword.checkEmailTitle')}</CardTitle>
            <CardDescription>{t('auth.forgotPassword.checkEmailSubtitle')}</CardDescription>
          </CardHeader>
          <CardContent>
