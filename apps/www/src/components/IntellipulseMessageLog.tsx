...
// Stub for EasyTip component
const EasyTip = ({ content, children }: { content: string; children: React.ReactNode }) => children;
// Stub for markdown components
const Pre = ({ children }: { children: React.ReactNode }) => <pre>{children}</pre>;
...
// ModelSelector import removed - component not available
...
export const IntellipulseMessageLog: FC<IntellipulseMessageLogProps> = memo(
        ...
        <div className="flex items-start">
          ...
          <div className="flex-1">
            ...
            {webcontainerActions.map((action, index) => (
              <CommandExecution key={index} action={action} />
            ))}
  ...
  (prevProps, nextProps) => {
    ...
    if (
      ...
      prevProps.onExecuteSteps !== nextProps.onExecuteSteps
    ) {
      return false;
    }
