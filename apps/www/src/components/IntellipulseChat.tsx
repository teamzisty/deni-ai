  ...
  */
    ...
    const handleImagePaste = useCallback(
      async (event: React.ClipboardEvent<HTMLDivElement>) => {
        if (
          !modelDescriptions[model]?.features?.includes("vision") ||
          !event.clipboardData ||
    ...
    const handleImageUpload = useCallback(
      async (file: File) => {
        if (!modelDescriptions[model]?.features?.includes("vision"))
          return;
        toast.promise(uploadImage(file), {
          ...
          success: (res) => {
            if (res.status === "success" && res.data?.url) {
              setImage(res.data.url);
              return t("common.upload.success");
            }
            throw new Error(t("common.upload.error"));
          },
          error: (error) => {
            return error.message || t("common.upload.error");
          },
        });
      },
      [model, modelDescriptions, uploadImage, setImage, t],
    );
    ...
    const handleImageUploadEvent = useCallback(
        ...
        <div className="flex w-full message-log visible">
          <div className="p-2 my-2 rounded-lg text-muted-foreground w-full">
            <div className="ml-3 animate-pulse">
              {modelDescriptions[model]?.features?.includes("reasoning")
