...
■ Coding Conventions
- Formatting/Linting: Adhere to Biome. Run `bun run lint` and `bun run format` before submitting changes.
...
- shadcn/ui generated files under `src/components/ui/*` are generally not edited and are excluded by Biome.
